...
import { verifyAccount, type VerifyRequest } from '@/src/lib/services/verify_service';
...
function applyCorsHeaders(res: NextResponse, origin: string | null): void {
  ...
  if (allowedOrigin) {
    ...
    res.headers.set(
      ...
      'X-Request-ID, X-Version, X-Timestamp'
    );
  }
}
...
export async function POST(req: NextRequest) {
  ...
  // Captura de token en Query Param (PROHIBIDO - Security Guard)
  // Segun Spec L385, si se detecta token en URL se debe rechazar con 400.
  ...
  try {
    ...
    const result = await verifyAccount(body, { 
      ...
      httpMethod: 'POST',
      tokenInQueryParam: queryToken || undefined
    });
    ...
    const res = NextResponse.json({
      ...
      message: 'El token de verificación es inválido o no fue encontrado.'
    }, { status: 400 });
    applyCorsHeaders(res, origin);
    return res;
  }
